...
class Filter:
    ...
    def __init__(self, sample_amount_for_calibration=200, imu_data=None):
        ...
        self.states_history = []
        self.full_history = imu_data
        self.full_history.loc[:, output_csv_columns] = None
        self.full_history.to_csv("accel_and_gyro_only.csv", index=False)
    ...
    def _initialize_state_x(self):
        ...
        # print(f"mean of accel {avg_accel}")
        # print(f"magnitude of accel {np.linalg.norm(avg_accel)}")

        # print(f"\nmean accel normalized {local_gravity_direction}")
        # print(
        #     f"magnitude of mean accel normalized {np.linalg.norm(local_gravity_direction)}"
        # )
        # print(f"calculated bias {self.accel_bias}")
        # print(f"magnitude of bias {np.linalg.norm(self.accel_bias)}")
        ...
        self.gravity_direction = avg_accel * constants.g
    ...
    def save_states_to_csv(self, filename):
        ...
        columns = output_csv_columns + ["time"]
